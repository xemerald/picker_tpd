/**
 * @file scnlfilter.c
 * @author Origin from: 981112 Lynn Dietz
 * @author Benjamin Ming Yang @ National Taiwan University
 * @brief Contains a function to filter messages based on content of site-component-network-loc fields.
 *        Works on messages with SCNL tags.
 * @date 2022-03-28
 *
...
/**
 * @name Local function prototypes & macros
 *
 */
static int          apply_filter( const char *, const char *, const char *, const char *, const void ** );
static int          filter_wildcards( const SCNL_Filter *, const SCNL_Filter * );
static int          remap_scnl( char *, char *, char *, char *, const SCNL_Filter * );
static char        *copytrim( char *, const char *, const int );
static SCNL_Filter *realloc_scnlf_list( const char * );
static int          compare_scnl( const void *, const void * );  /* For qsort & bsearch */
/**
 ...
 * @note This function may exit the process if it finds serious errors in any commands
 */
...
/* */
	return apply_filter( sta, chan, net, loc, outmatch );
}
...
int scnlfilter_remap( char *sta, char *chan, char *net, char *loc, const void *match )
{
	...
	/* Rename it by the SCNL from match pointer */
		return remap_scnl( sta, chan, net, loc, _match );
 ...
 * @param outmatch
 * @return int
 * @retval the priority if SCNL is in "send" list otherwise EW_PRIORITY_NONE (0) if not found
 */
static int apply_filter( const char *sta, const char *chan, const char *net, const char *loc, const void **outmatch )
{
	int          i;
 ...
 * @return int
 */
static int remap_scnl( char *sta, char *chan, char *net, char *loc, const SCNL_Filter *filter )
{
	uint8_t fbit;
...
/* */
	for ( fbit = STA_FILTER_BIT; fbit > 0; fbit >>= 1 ) {
		if ( !(filter->rwilds & fbit) ) {
			...
			case STA_FILTER_BIT:
				if ( sta )
					memcpy(sta, filter->rsta, TRACE2_STA_LEN);
				break;
			case CHAN_FILTER_BIT:
				if ( chan )
					memcpy(chan, filter->rchan, TRACE2_CHAN_LEN);
				break;
			case NET_FILTER_BIT:
				if ( net )
					memcpy(net, filter->rnet, TRACE2_NET_LEN);
				break;
			case LOC_FILTER_BIT:
				if ( loc )
					memcpy(loc, filter->rloc, TRACE2_LOC_LEN);
				/* No break here */
			default:
				break;
			}
...
/**
 * @brief This function is passed to qsort() and bsearch(). We use qsort() to sort the station list by SCNL numbers,
 *        and we use bsearch to look up an SCNL in the list if no wildcards were used in the requested channel list
 *
 * @param s1
 * @param s2
 * @return int
